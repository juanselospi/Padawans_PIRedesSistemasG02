...
// Puerto UDP de descubrimiento entre nuestro servidor y nuestro intermediario.
// El intermediario pregunta "LEGO_DISCOVER" y el servidor responde "LEGO_SERVER <puertoTCP>".
...
/**
 * Respondedor de descubrimiento UDP.
 *
 * Escucha en PORT_SERVER_DISC y, cuando recibe "LEGO_DISCOVER" de un intermediario,
 * responde por unicast "LEGO_SERVER <puertoTCP>" al remitente. Solo responde cuando le
 * preguntan (no hace broadcast espontaneo), por lo que no genera trafico en reposo.
 * Esto permite que servidor e intermediario se descubran sin importar cual arranca primero.
 */
void LegoServer::discoveryResponder()
{
   int sockfd = socket(AF_INET, SOCK_DGRAM, 0);
   if (sockfd < 0)
   {
      std::cerr << "discoveryResponder: no se pudo crear socket UDP" << std::endl;
      return;
   }
   ...
   addr.sin_port        = htons(PORT_SERVER_DISC);

   if (bind(sockfd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
   {
      std::cerr << "discoveryResponder: no se pudo bind UDP " << PORT_SERVER_DISC << std::endl;
      close(sockfd);
      return;
   }

   std::cout << "Descubrimiento UDP del servidor escuchando en puerto "
             << PORT_SERVER_DISC << std::endl;
   ...
   while (true)
   {
      ...
      if (strncmp(buf, "LEGO_DISCOVER", 13) != 0)
         continue;

      std::string reply = "LEGO_SERVER " + std::to_string(port);
...
void LegoServer::run()
{
   ...
   // Responder a descubrimientos del intermediario (orden de arranque independiente).
   std::thread discThread(&LegoServer::discoveryResponder, this);
   discThread.detach();
